...
PARAMS = dict(
    ...
    material=dict(
        ...
        # [D85 채택 · REG_AUDIT §6.1 (2)] **판석 톤 완화** (0.74, 0.73, 0.70) →
        #   (0.66, 0.651, 0.624). §12-10 (3) 이 미리 지목해 둔 레버(`plaza_tint`
        #   0.74 → 0.66)를 그대로 발동한 것이고, 실측이 근거를 확정했다: 순백
        #   `frac(min>0.8)` **최대 0.2809 · 그 92.7 %가 `Plaza/Approach` 판석**
        #   [260823_v3p5_h12reg_A 16컷]. **광장을 다시 꾸미지 않는다** — 세 채널을
        #   **같은 배율 0.8919 로 균등 축소**해 색상비(1.000 : 0.9865 : 0.9459)를
        #   소수 넷째 자리까지 보존했다. 즉 색조는 그대로고 톤만 내려간다
        #   (sceneH3 `rock_tint`·`brick_tint` 레버와 같은 규율 = §7.1 색상비 고정).
        plaza_tint=(0.66, 0.651, 0.624),    # 밝은 화강석 판석 (알베도 0.30 대)
