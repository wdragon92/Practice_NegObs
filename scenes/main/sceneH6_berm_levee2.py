...
표준법 (법1–법8) 준수 메모
  ...
  법4 기능 필수성      — 난간(2.80 m 낙차 = 건축법 시행령 §40 의무 구간) · 볼라드(제방 진입 차량 억제) ·
...
PARAMS = dict(
    ...
    tactile=dict(x0=-1.50, x1=-1.10, y_half=8.6, proud=0.004),
    # [규격] 점형블록 세로 0.40 m — 교통약자법 시행규칙 별표2. 코핑 앞 0.37 m 이격.
...
GATED_ZONES = [
    ...
    ("Tactile",      "cue_tactile",        -1.50,  -1.10,  -8.70,  8.70, 0.004),
...
def main():
    ...
    def build_tactile(M):
        ...
        print(f"[cue] tactile ON — 점형블록 세로 {t['x1'] - t['x0']:.2f} m · "
              f"코핑 앞 이격 {HZ['coping']['x0'] - t['x1']:.2f} m · proud {t['proud']} m")
