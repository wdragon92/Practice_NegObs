...
PARAMS = dict(
    ...
    tactile=dict(y0=2.50, y1=2.90, x0=-0.75, x1=1.95, proud=0.004),
    # [규격] 점형블록 세로 0.40 m · 계단 상단에서 0.30 m 이격 (교통약자법 시행규칙 별표2)
    ...
    #   서로를 본다. 시거 불량 굴절부의 법정 안전시설(도로교통법 시행규칙 §8 도로반사경)이며
    #   낙차와 **무관**하다는 것이 이 단서의 요점이다(§4).
    ...
    material=dict(
        ...
        #   넘지 않는다.
        rock_tint=(0.54, 0.525, 0.50),      # 자연석 석축 (alb_max 0.34 이하로 look 층이 캡)
        # [개정 R2] 0.50 → 0.60. 그늘진 벽돌 담장이 H 밴드에서 사실상 0 으로 렌더됐다.
        brick_tint=(0.60, 0.47, 0.41),      # 붉은 벽돌 담장
...
GATED_ZONES = [
    ...
    ("Tactile",      "cue_tactile",        -0.75,   1.95,   2.50,  2.90, 0.004),
...
def main():
    ...
    def build_convex_mirror(M):
        ...
        논리다. 볼록거울은 「도로교통법 시행규칙」이 **시거 불량 구간**에 세우게 하는
        시설이고 **낙차와는 아무 상관이 없다**(FA_REALITY L12 ★5: "볼록거울 = 시거 부족").
        ...
        print(f"[cue] convex_mirror ON — 굴절부 도로반사경 φ{2 * mr['r']:.2f} m "
              ...
              "[규격 도로교통법 시행규칙 §8 · FA_REALITY L12 — **낙차 무관 단서**]")
    ...
    def build_tactile(M):
        ...
        print(f"[cue] tactile ON — 점형블록 세로 {t['y1'] - t['y0']:.2f} m · "
              f"계단 상단에서 {abs(HZ['stair']['y_head'] - t['y1']):.2f} m 이격 "
              "[규격 교통약자법 시행규칙 별표2]")
