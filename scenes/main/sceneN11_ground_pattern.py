...
CUE_CLASS = {
    ...
    # [D82 ⓐ 근거 조항] 「교통약자의 이동편의증진법 시행규칙」 별표2 **제7호** —
    #   자동차 진입억제용 말뚝 **앞 0.3 m** 에 점형블록. **점형 = 경고 1밴드**이고
    #   이 씬에는 유도가 필요한 분기·대기 지점이 없어 **선형블록은 두지 않는다**.
...
SCENE_CONFIG = {
    ...
    "cue_railing":         False,
    "cue_tactile":         True,   # 진입 볼라드 전면 점형블록 [교통약자법 시행규칙 별표2]
...
PARAMS = dict(
    ...
    tactile=dict(
        # 진입 볼라드 전면 점형블록 — [규격 교통약자법 시행규칙 별표2: 볼라드 전면 0.30 m]
        ...
        #   [규격 「교통약자의 이동편의증진법 시행규칙」 별표2 제7호 — 자동차 진입억제용
        #    말뚝(볼라드) **앞 0.3 m** 에 점형블록을 설치한다. 깊이 0.30 m = 1유닛]
...
def main():
    ...
    def build_tactile(M):
        ...
        print(f"[cue] tactile ON — 볼라드 전면 점형블록 **{len(t['bands'])}개소**"
              ...
              f"{t['proud'] * 1000:.0f} mm [규격 교통약자법 시행규칙 별표2]")
...
def _geometry_selfcheck():
    ...
    chk("점형 밴드 깊이 0.30 m = 1유닛 [별표2]",
        abs(tb[0][2] - tb[0][0] - 0.30) < 1e-6, f"{tb[0][2] - tb[0][0]:.2f} m")
    chk("점형 밴드가 볼라드 열 앞 **0.30 m** [별표2 제7호]",
