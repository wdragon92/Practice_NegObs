...
def main():
    ...
    def build_skyline(M):
        ...
        #   scene17's placebo (PREREG_CUEOFF sec.3.2).  These stand 86-98 m away
    ...
    def build_dressing(M):
        # [CUE-OFF arm C] the terrace is filled to z=0 by `build_flat_fill`, so
        #   every terrace-anchored dressing object rides the fill instead of
        #   being buried under it (sceneC2:666 `Z_LOW` switch, same reasoning).
        #   With the flag off this is the canonical `terrace.z_top` and every
        #   expression below is byte-unchanged.
        #   DECLARED LIMIT (PREREG sec.7-4): arm C preserves the dressing OBJECTS,
        #   not the crest-occlusion RELATIONSHIP that made a terrace lamp pole a
        #   "column with a hidden base" -- with the hazard gone there is no crest
        #   to hide anything, and that is intrinsic to a no-hazard arm.
