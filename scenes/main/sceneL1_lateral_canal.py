...
PARAMS = dict(
    ...
    #   재질·단수·피치가 모두 다르다.
    tactile=dict(bollard=(-13.85, -4.00, -8.85, -3.60),   # 볼라드 전면 0.30 m
                 crest=(-7.70, -2.60, -7.30, 2.60),        # 2단 상단
                 proud=0.004),
...
GATED_ZONES = [
    ...
    # 점형블록 2밴드 — 볼라드 전면(광장 남측)과 2단 상단(스트립 안, proud 4 mm).
    ("Tactile-Bol",  "cue_tactile",       -13.85,  -8.85,  -4.00, -3.60, 0.004),
    ("Tactile-Crest", "cue_tactile",       -7.70,  -7.30,  -2.60,  2.60, 0.004),
...
def main():
    ...
    def build_tactile(M):
        ...
        print(f"[cue] tactile ON — 볼라드 전면 밴드 세로 {by1 - by0:.2f} m "
              f"(볼라드 열에서 {abs(PARAMS['bollard']['y'] - by0):.2f} m) + "
              f"2단 상단 밴드 세로 {cx1 - cx0:.2f} m · proud {t['proud']} m "
              "[규격 교통약자법 시행규칙 별표2 — 볼라드 전면 0.3 m · 계단 상단]")
