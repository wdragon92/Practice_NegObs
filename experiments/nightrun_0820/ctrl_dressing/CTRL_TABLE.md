...
**old off** = `dataset/260819_main_off` (the toggle also deleted the leaf mound, the railing and, in N3, the mural) · **new off** = `dataset/260820_ctrloff` (`keep_dressing`: hazard geometry only). Both are paired against the SAME on arm, `dataset/260819_main_on`. Every off frame carries an all-zero GT, so `FA_frame` is a pure false-alarm rate.
