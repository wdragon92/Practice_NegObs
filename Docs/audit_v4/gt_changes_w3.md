...
차기 라운드에서 육안.
**§4 착지기록** (2026-08-15): 6건 전량 시공(①~⑤ 병렬 5기+본선, ⑥ 본선 — s17 은
에이전트 병합 메시 위에 배터 마진 0.30→0.0 후속 1건). 셀프체크 전 건 PASS(낙차
에지·보행면 z·hazard AABB 비트 불변 증빙). 검증 렌더 `260815_w4_hzbatch`(13씬,
GPU 중단 1회 → hzbatch2 재개 완주). 육안: ① stair_down — 관통 검은 홈이 랩 음영선
(지그재그 그림자)으로 전환, 순흑 소멸 ② 쐐기 폐합·부유 판 소멸 ③ lower_lookback —
눈 캡 3부재(베드·슬래브·45° 코)가 융기선으로 판독, 노징 노출 6 mm(수용 출하 —
...
Round: 차기 검증 라운드에 s16 포함.
**§4 착지기록** (2026-08-15): `build_anchored_stains` + s16 3종(낙수선 3로브·연석
토사 2로브·볼라드 베이스 drip 6) 시공, 재질 = plaza_lower 텍스처 암틴트 2종(M["adrip"]/
M["asoil"] — 상수색 리본 아님). hzbatch 육안: 저대비 성립(의도 — ㎡ 소수 밀도·앵커
설명 가능 위치), shadow_band 컷에서는 원거리라 미세 판독. **확산(s11 등)은 파일럿
갤러리 사용자 검수 후 별행** — 자율 런 중 미착수로 판단(데칼 정온 룰링의 opt-in
갈래인 만큼 파일럿 검수가 선행). **Status: OPEN (착지 — 검수 대기, 확산 보류)**
...
**§4 착지기록** (2026-08-15): s02(벽 타일 대역·트로퍼 가시 기구화)·s15(바닥 0.115→
0.30·회벽 ×1.50·패치 동행) 시공. hzbatch 실측(shortcut_audit, 판정컷 h0.3_d5):
s02 dark 0.8→0.0 %·clipHi 34.7→30.4 % / s15 dark→0.6 %·clipLo 18.2→1.6 % —
**단 s15 clipHi 2.5→52.7 % 과보정 실측** → 2차 반복 커밋 b99eca4(바닥 목표 0.30→
0.24 대역 중앙, 패치 +12 % 동행). 코퍼스 r_pb(낙차-암부 점이연 상관): dark 0.130 ·
deep 0.161, 낙차군 지면대 암부 중앙값 0.017 vs 무낙차군 0.047(역전) — "깊을수록
어둡다" 탈상관 축의 정량 착지. s15 2차분은 최종 allview 라운드에서 육안.
**§4 2차 착지** (2026-08-16, round `260816_w4_final33_on`): s15 바닥 0.24 실측 —
clipHi 52.7→**32.9 %**(dark 0.6·clipLo 2.4 유지, 골목 판독 성립). 잔여 32.9 는 노출
기구(GT-126 ⓐ) 귀속 — 알베도 하향으로는 더 못 내림(대역 하한 접근). **Status: OPEN
(착지 — 검수 대기)**
...
**§4 착지기록** (2026-08-15): `_near_frame`(lod_dist≤30)+`_reveal` 4부재(_FW 0.06/
_FP 0.03) 시공. hzbatch 육안: N1 band_grazing — 근경 동 창호가 프레임 있는 개구로
판독(맨 유리 직부 소멸), N3·s16 동반 발현. OFF 팔 비트동일(geom 33/33). regr FRAME
선언 귀속. **Status: OPEN (착지 — 검수 대기)**
...
**GT 판정**: R-2(재질 전용).
**§4 착지기록** (2026-08-15): apply_ground 접힘 경고 계기 + s16(M["ironwork"] —
manhole/gully/gutter_cover/trench_frame)·s21(_iron 동일 패턴) 시공. hzbatch 육안:
s16 포털 컷 우하 맨홀·s21 광장 맨홀 2기 — granite_dark 소축척 결이 있는 암회 주철로
판독("스티커" 소멸). marking/weed/stain 군 분리는 후속 별행.
**Status: OPEN (착지 — 검수 대기)**
...
**§4 착지기록** (2026-08-15): make_pbr blend 관통(기본 None 비트동일) + s10 사면
잔디 A=grass_lawn/B=dirt_park @2.5 m 모틀 시공. **1차 렌더 실측: 창백 사구화**
(틴트 (1.85,1.30,1.75)+블렌드 0.35 — 극단 틴트 완화가 과도) → 2차 반복 커밋
a91b83b(틴트 (1.32,1.18,1.02) 녹색 잔존 대역·블렌드 0.28). 파고라 구조·쐐기 폐합은
동일 컷에서 성립 확인(GT-119② 참조). 2차분은 최종 allview 라운드에서 육안.
**§4 2차 착지** (2026-08-16, final33): 육안 — 사면이 올리브 휴면 잔디+고사 반점
모틀로 판독(창백 사구 소멸), clipHi 1.6·dark 1.8 %. **Status: OPEN (착지 — 검수 대기)**
...
**§4 착지기록 + 판정** (2026-08-15, round `260815_w4_physpilot` 44컷 A/B): **전면
채택 기각 — 기전이 있는 부정 결과.** ① ps_slope 는 전 컷 ±0.1 이내(s03 은 오히려
−0.09~−0.27 악화) — **램버시안 확산은 slope 잔여 격차의 주인이 아니다**(감사 후보
1순위 폐기; "판정창 대역 내 3 % RMS 알베도 주입"이 잔존 1순위로 승격). ② 밝은 광물
면에서 Oren-Nayar 역산란 증광(flat-moon)이 정오 클립 기구(GT-126 ⓐ)와 정면 충돌 —
s16 clipHi 8.5→30.2 %·21.9→48.8 %·28.4→52.5 %, s03 도 동반 상승. ③ 유일한 순이득
= 어두운 asphalt(실효 0.10)의 h0.3 광택 감소(s13 clipHi 2.2→1.2·2.1→1.6) — grazing
0.25 단독의 asphalt 한정 채택만 미래 후보로 기록. 팔은 opt-in 존치(기본 OFF 비트동일,
노출 재예산 실험용). 부속: 데이터 롤 r_pb(120컷·5씬 — scene04/16 vs N1~N3) dark
+0.389/clipHi −0.429 실측이나 **씬 정체성 교란**(씬 5개 표본 — N1 광장 단독 지배) —
코퍼스 판정컷 r_pb 0.13/0.16 이 유효 수치, 데이터 롤 정측정은 전 씬 데이터 렌더 필요
(GPU 예산 = 사용자 판정). **Status: CLOSED (기각 — 팔 존치·값 승격 없음)**

## 82. GT-126 — 잔여 감사 확정분 일소 배치: 미착수 8씬 + 순백 클립 꼬리 3씬 (선신고)

**Authority**: 08-14 사용자 3일 자율 위임(행 69). 근거 = 감사 valid-new 중 GT-115/119
미배정 씬 잔량(높음 33·중간 31) + r4batch 실측 clipHi 꼬리(s14 **80.2 %**·s05 61.6·
s09 56.8·s18 45.6 — "죽은 픽셀 톤 양끝" 감사 실측의 반대쪽 끝). 건물 재작업 큐 씬
(s01·s11·s15·s21·N3)은 **제외**(사용자 판정 대기 존중), sceneC4 암부 36.2 %는 수막
룰링 인접이라 관찰만.
**Scope**(선신고 — 씬 파일 단독 소유 병렬, 공유 파일 무수정):
① **s04**: 지면 판 부유 개방 슬롯(**HAZ — 낙차 에지·답면 z 비트 불변 셀프체크 의무**)
   + 민둥 기둥 나무 실자산화. ② **s07**: 돌계단-석축 관통(**HAZ 동일 의무**) + 석축
   타일 반복(unit_cell 지터 — GT-113 W3 어휘) + 원경 산지 배경막. ③ **s14**: 테라스
   순백(판정컷 clipHi 80.2 % — 실효 알베도 실측 후 대역 복귀). ④ **s05**: clipHi
   61.6 % 동일 처방. ⑤ **s09**: clipHi 56.8/clipLo 14.8 양끝. ⑥ **s18**: 모래
   clipHi 45.6(건사 실측 대역 0.35~0.45 상한). ⑦ **s20**: 계절 모순(하계 수관 vs
   가을 낙엽 카펫 — 소산 방향)·파라펫 순백 카드·포장 UV 스미어링. ⑧ **D3**: 노견
   마른 풀 물증·차도 아스팔트 판독 복귀·원경 상자 폐합. ⑨ **N4**: 45 m 옹벽 분절
   (s08 GT-115⑩ 레시피 준용 — 신축이음·배수공·갓돌)·백지 명판 처분(**문안 신설 금지**
   — 제거 또는 무문자 프레임만)·능선 상수색 벽. ⑩ **N5**: 광장 순백 대역·그레이팅
   슬롯 바닥 어둠 복귀. ⑪ **s06**(한정 소유: 립·이음·코벨·유리 접촉 금지 — 08-14
   재지시 주제는 사용자 몫): 콘크리트 흰 인클루전 격자 반복 해소 + 육교 순흑 그림자
   원인 진단(재질·텍스처 원인일 때만 수정, 조명 불변).
**GT 판정**: R-1(HAZ 2씬 셀프체크 증빙) + R-2/R-3. 사람·차량 금지, 사인·문안 신설
금지, 낙차 에지 불변. 검증 = floor 4종 + 최종 allview 라운드 육안 + regr 선언 귀속.
**Scope 추기(2026-08-15 — ⑦ 진단의 코퍼스 파급분)**: ⑫ s20 이 실측한 `Looks/Band`
정확일치 paint 핀 오라우팅(화강 밴드가 재질 기계 전체를 놓침)의 동종 3건 — s14·s05·
s21 의 **텍스처 밴드**(band_dark/granite_dark) 경로만 `Looks/BandDark` 로 개명(콘크리트
계열 복귀). s16(0.05 암밴드)·s13(0.72 반사 밴드)은 도장 밴드가 맞아 존치.
**§4 착지기록** (2026-08-15): 12항 전량 시공(병렬 11기 + 본선 ⑫). HAZ 2씬 증빙 —
s04: 프로브 불변절 diff 공란(레지스트리·카메라 13·보행/콜라이더 AABB 51 비트동일,
신규 = 크레스트 매몰 슬랩 4·상단 침목 1 전부 collider=False) / s07: 동일(신규 = 뺨돌
1·능선 분절 +28, 답면·베드·커브·카메라 14 불변). 주요 발견: ⓐ **정오 노출 기구 —
씬별 게인 1.77~2.46 실측, 클립선 = 실효 알베도 0.21~0.27 로 클래스 천장 0.34 아래**
(s14 80.2 %·s05 61.6·s18 45.6·s09 56.8 의 공통 기전; 밴드 내 값도 클립. 천장 재예산
vs 노출 조정은 코퍼스 광학이라 **사용자 판정 후보**로 이월) ⓑ 분류기 함정 신규 4종 —
bare Band 정확핀(paint)이 화강 밴드 포획(s20+⑫ 4씬 in-file 정정), DeckConcrete→wood
(s06 판정면 — 관찰만), Pole→metal(D3 목재 전주 — P-11 대기), ridge/crest→콘크리트
캐치올 streak(s07 in-file 정정) ⓒ Lombardy_Poplar 자산이 Isaac/RTX 에서 트렁크만
렌더(3라운드 교차 확인 — 원거리 전용 취급 권고, s04 는 birch/elm 대체) ⓓ s06 "흰
인클루전" = 폼타이 노멀 딤플의 태양 글린트(알베도 아님 — 슬래브 트로웰 텍스처 전환,
진짜 거푸집면은 존치+유닛셀 지터) ⓔ N5 슬롯 밝음 = 차광 증명 오류(막대층 20 mm 만
차광 — 섬프 −0.25 m 신설, geocheck 전부 녹색). s09 암부 꼬리 13.8 pp 는 인스턴스
수목 프로토타입 내부(씬 사정거리 밖 — T4b 랩퍼 필요 기록). 검증 floor: py_compile
...
처분은 사용자 판정(§5-1). s04 55.1/26.6/22.2 → 60.1/**1.4**/**1.2**(심암부·표시역
붕괴 해소 — dark 는 수림 그늘 정상), s07 65.0/57.1→17.7/15.8. 육안 성립: s04 버치
실수목+크레스트 폐합 · s06 트로웰 슬래브(펄 소멸) · s09/s05/s20 판독 회복 · N4 분절
발현 · N5 섬프 암선 · D3 노견 건초화·배경 재단. **육안 잔여 3건**(후속 후보로만 기록):
...
`260816_w4_w8deck`(s06·s18) 완료. **§4 육안 완결** (2026-08-16, A=`260816_w4_final33_on`
@e02f362 핀 이전 / B=`260816_w4_w8deck` @a513cad 핀 이후 — 스탬프로 전후 확정):
① **s06 판정 데크 슬래브 = 콘크리트 판독 성립.** `deck_entry` 에서 판정면이 트로웰
그레인 있는 회색 콘크리트로 읽히고 목재 기미 없음. 변화 픽셀 7.75 %(`deck_entry`)·
2.66 %(`preset_h1.8_d2`), 변화역 평균 RGB 0.698/0.691/0.647 → 0.686/0.678/0.634
(−0.012 미세 하향, 지면 처방 복귀분). **회귀 없음** — 계단·유리·난간·식생 전부 불변.
② **s18 원경 교량 상판 = 의도 방향 이동, 단 화면 기여 5행.** 상판은 `sea_beauty`
에서 **행 423~427 = 5 px** 만 차지한다. 피크 행(425) 휘도 **0.853 → 0.781(−0.072)**,
행 424 0.759→0.707 · 행 426 0.793→0.743 — **순백 목재조 → 회색 콘크리트조**로 하향.
그 외 전 화면 변화는 0.03~0.45 % 픽셀·Δ0.0015~0.005 = GI 리바운드 잡음 수준.
즉 **핀은 정확히 상판에만 걸렸고 부작용 0**이나, s18 에서의 룩 기여는 구조상 미미하다
(원경 5행). ③ 두 씬 모두 신규 결함·회귀 미발견. **Status: CLOSED (착지 — 육안 완결,
사용자 검수는 갤러리에서)**

## 85. GT-129 — 신선 감사 확정 high 회귀 수리 배치: 6씬 (선신고)

**Authority**: 08-14 사용자 3일 자율 위임(행 69) + 08-16 사용자 재개 지시.
근거 = `260816_w4_final33_on` 기준 신선 전수 감사(원자료
`reports/raw_260816/fresh_audit_final33_result.json`, 116건 중 confirmedHigh 10건)
의 픽셀 검증 완료분. 이 중 **자율 런 수리가 만든 회귀**(regression_from_recent)를
우선 일소한다. 건물 재작업 큐 씬 중 s15 는 창 리빌 프레임이 GT-122 오착지라 **결함
수리 한정**으로만 포함(디자인 변경 금지). s17·s21·N4 의 confirmedHigh 3건은 본 배치
범위 밖 — 다음 큐로 기록.
**Scope**(선신고 — 씬 파일 단독 소유 병렬, 공유 파일·킷 무수정):
① **s05**(HAZ — 무대 아크 계단): 아크 계단 끝단이 무대 바닥을 관통해 솟은 예각 칼날
   쐐기 2기(`pt_noon_rim_view.png` 180,595,290,760 및 우측 대칭) + 아크 계단↔포디움
   접합부의 Z자 개방 흑슬롯(560,850,670,990 / 1255,825,1450,1035, lum<0.02 2.5~2.6 %).
   GT-11 의 "진성 환형 섹터" 재시공이 이음을 다시 연 것. **낙차 에지·보행면 z·hazard
   AABB 비트 불변 프로브 증빙 의무.**
② **s10**(HAZ — 회랑 사면 기저): 사면 웨지와 하부 잔디/지형 슬래브 사이 개방 이음으로
   **스카이박스가 비침**(`preset_h1.8_d2` 0,565,560,835 코어 RGB 0.552/0.592/0.633,
   std 0.004~0.028 = 하늘색 계열 상수면; h1.8_d5·h1.8_d10·h0.9_d2 재현). 월드 구멍.
   **동일 프로브 증빙 의무.**
③ **s04 — 철회(사용자 지시 08-16, 시공 전 정지)**: 사용자 최애 씬으로 **수정 보류**.
   GT-129 및 이후 전 단계에서 **제외**한다(작업 미착수·워킹트리 무변경 확인). 이미
   착지한 GT-126 ① 의 s04 변경은 **되돌리지 않는다**(환원 여부는 사용자 판정).
   원 결함 기록만 남긴다 — 낙엽 지면을 가로지르는 순흑 개방 심 폴리라인(`preset_h0.3_d10`
   780,420,1400,560 — lum<0.02 2.2 %, min 0.000; h0.9_d10·h0.3_d2 재현). GT-126 ①
   크레스트 매몰 처리의 **밖에 남은** 슬래브·드리프트 로브 경계 — 매몰/스커트 연장.
④ **s06**(한정 소유 — 데크 판재·립·코벨·유리·난간 무수정, 08-14 사용자 주제 존중):
   교각 다리가 **옆으로 어긋난 드럼 스택**(`pt_noon_overview` 1160,520,1320,820 —
   5단 실린더가 지름의 15~25 %씩 측방 이동, 이음마다 타원 캡 림; 두 번째 교각은 하단
   드럼이 기초 패드 밖으로 돌출). GT-115 분절 시공 오착지 — 세그먼트 중심 단일축 정렬.
⑤ **s15**(결함 수리 한정): 골목면 창 프레임이 서로 겹치고(중간 프레임 선대가 이웃 창
   유리 앞을 지남) 기울기가 창마다 다름(인접 10°, `narrow_up` 20/10/5°), 벽면에서
   떠 있음. GT-122 `_near_frame`/`_reveal` 오착지 추정 — **원인이 공유
   `scene_common.build_building` 이면 수정 금지·진단만 보고**하고 씬 내 완화만.
⑥ **s03**: 수면 평면 원단이 직사각 톱니(`meander_air` 600,400,1300,480 — 13치, 피치
   ~55 px; `bank_oblique`·`river_along` 재현). v5.1 사행 재시공의 19개 축정렬 수면
   밴드 끝단 노출 — 밴드 연장 또는 원경 차폐. **수면 z·hazard 기하 불변.**
**GT 판정**: R-1(HAZ 2씬 프로브 증빙 — 낙차 에지·보행면 z·hazard AABB·판정 카메라
비트동일) + R-3(OCCL/GRAZE 재스탬프). 사람·차량·사인·문안 신설 금지. 검증 = floor
4종(py_compile · SMOKE · geom 33/33 · lint A/B Δ0) + 검증 라운드 `260817_w4_regfix`
6씬 육안 + regr 선언 귀속 + gallery.
**§4 착지기록 — 코드분** (2026-08-16, commit `e469c32`, 5씬 시공·s04 철회):
① **s05** — 두 결함(칼날 쐐기·Z 흑슬롯)이 **동일 근원의 수치 오류**로 판명.
`Stage/Podium` 은 raw `UsdGeom.Cylinder`(r 3.0)인데 W3 L05 가 겹침을 **n=32**
(처짐 14.4 mm) 기준으로 잡아 `under=0.10` 을 줬다. UsdImaging 의 실제
`numRadial` 은 **10**(십각형, 아포뎀 2.85317, 처짐 **146.8 mm**) — `r_in=2.900` 이
**그려진 벽 바깥**에 서는 구간이 패싯 중점(126/162/198/234°) 둘레 ±10.31° 씩
생겨 최대 **46.8 mm 초승달**이 무대 원반 상면(−1.207)까지 **0.354 m 깊이로 개방**됐다.
네 패싯 중점이 전부 접근계단 아크(127~163 / 197~233) 안에 든다. 픽셀 대조:
162° 초승달 예측 x 460…642 대 실측 흑성분 **477…653**(대칭 1265…1439), 126° 는
`rim_view` 접선 방위에서 9.5° 어긋나 **접선 슬리버 = 칼날**로 투영되고 136.31° 에서
닫힘(예측 x 294, 실측 288). **구멍은 애초에 시공된 적이 없다 — 묻으려던 다각형보다
묻을 대상이 넓었을 뿐.** 처방 = `under` 0.10 → **0.35**(아포뎀 안쪽 203 mm) +
`cyl_radial=10` **선언 신설**(다시는 추정 n 으로 겹침을 잡지 못하게) +
`podium_step_selfcheck` ③ 이 선언값을 읽도록 수정, 게이트 **⑦ 바닥 관통** 신설.
**프림 0 증감(1907→1907), 수정 28행 전부 extent z 비트동일**(z_min/z_max 변경 0행) —
움직인 것은 이미 포디움 솔리드 내부인 안쪽 x/y 모서리뿐. 보행면 재확인:
포디움 상면 −0.853 · 상단 답면 −0.856 · 하단 답면 −1.028 · 뺨 −1.031 · 에이프런
−1.203 · 무대 바닥 −1.207 **전부 불변**, tier 낙차 에지·엔트리 비행·립 커브는
diff 에 **등장조차 안 함**. 광선 프로브: 초승달 공극 **26 px → 0**.
② **s10** — 공동은 감사가 지목한 남측이 아니라 **북측**. `ybank()` 가 만드는
`Bank_NorthBank` 는 **자기 상면에 수직으로 두께가 뻗는 rotX 슬래브**라 발치가
맨 모서리이고, `CorridorSlope_*` 는 상면 **아래로만** 재질을 갖는다 →
**{y=8.00, corridor_z(x) ≤ z ≤ 0.000}, x −1.50…44.10 을 어떤 프림도 저작하지 않는다.**
`h1.8_d2` 픽셀 (375,710) 광선이 y=8.00 을 x=20.0, z=−4.44 에서 통과해 월드를 나간다.
**씬은 이미 알고 있었다** — `deck_module_selfcheck` 가 이 공동을 "어떤 카메라에서도
보이지 않는다"고 출력했는데, 60° hFOV 에서 d2 눈의 북측 한계는 |y| < 0.577·(x+2) 라
y=8.00 을 x=11.9 에서 통과, **북측 림 12 m 가 전 그리드 프레임 안**이다(전제 오류).
처방 = `Plate_NorthCutFill` **1프림 additive**(grass, `col=False`, 전 수치를 기존
datum 에서 유도) + 게이트 신설(y=8.00 평면 32 841점 관통 프로브, 빈 점 0).
프로브 diff **추가 1 · 제거 0 · 수정 0** — 기존 3058행 **바이트 동일**. 14개 저작
뷰 재추적: 수평선 아래 하늘 픽셀 **4319/1787/1333/387/48 → 전부 0**.
③ **s06** — 감사의 "측방 어긋난 드럼" 가설은 **반증**(드럼 중심 편차 실측 **0.000 m**).
진짜 결함은 **등반경 드럼 5단이 테이퍼를 흉내**내며 이음마다 **26 mm 반경 렛지**를
남긴 것 — 그 조명 받은 캡 타원이 "어긋난 드럼"으로 읽혔다. 기초 패드 이탈 주장도
반증(사방 여유 0.23 m). 처방 = 단일 테이퍼 프러스텀 메시. 프림 1538→1522.
데크 판재·코벨·립·유리·난간 **비트동일**(사용자 소유 주제 무접촉 증명).
④ **s15** — 공유 코드 아님(씬 로컬 `_opening`/`build_house`; s15 는
`build_building` 을 호출하지 않는다). 창 중심이 `cx ± 0.28·w` 로 **고정비 하드코딩**
이라 `pier = 0.28·w − 0.92` 가 **w < 3.286 에서 음수** — 좁은 5동(w 2.30~3.20)에서
피어 **−24 ~ −276 mm** = 상호 관통. 화면 겹침 실측 `preset_h1.8_d2` +54.0/+60.3 px,
`narrow_up` +206.9/+68.7 px. **감사의 두 부수 주장은 오독**: (a) 기울어짐 — 어떤
`Win_*`/`Door_*` 도 RX/RY/RZ 를 갖지 않고 전 선대가 단일 소실점을 공유("부채살 20/10/5°"
는 관통 박스들의 돌출 측면), (b) 벽면 이격 — 프레임은 셸에 **20 mm 매입**(간극 0),
보이는 것은 의도된 25 mm 유리 리빌. 처방 = `facade_openings()` 신설(여유 유도식),
넓은 파사드는 **비트동일**, 좁은 파사드는 문+창 1 로 폐합. 프림 743→723.
**⚠ 사용자 판정 대기**: 좁은 4동의 창이 2 → 1 로 줄었다. 물리적으로 문+창2+피어가
들어가지 않아 내린 결정이나 **파사드 디자인 변화**이고 s15 는 건물 재작업 큐 씬이다 —
되돌릴지 유지할지 결재 필요(§5 참조).
⑤ **s03** — `river_band` 가 **각 서브밴드 자기 중심을 피벗**으로 회전시켜 끝 모서리가
서브밴드마다 `w·tan(yaw)` 씩 이동·복귀 = 톱니. 말단 seg(yc=+45.0, yaw=+32.274°)에서
**12치**(3+4+5, 감사의 "~13"). 투영이 감사 크롭과 정확 일치. **감사 제안("far-bank
선 너머로 연장")은 불가** — 원경에 y≈48.85 너머 모델이 없어 톱니는 **하늘을 배경으로**
선다. 처방 = 밴드별 무yaw `WaterEnd_{0,1,2}` 캡 **3프림**(수면 8.5 mm 아래, 단일 z).
프로브 diff **추가 3행·제거 0·수정 0**, 456개 `Water_*` 전부 비트동일,
**수면 z −3.35 before == after**. 실루엣 반전수 **23 → 1**(3컷 전부),
판정 9프리셋에서 캡 코너 **0/12** 노출.
**검증 floor 실측**: py_compile 5/5 · SMOKE 5/5 exit 0 · `geom_invariance_check.py`
**R-4 33/33 · R-6 33/33 PASS** · `placement_lint.py --scenes scene03,05,06,10,15`
A/B(stash) **ERROR 6→6 · WARN 56→56 · BLOCKED 4→4 = Δ0 신규**(잔존 ERROR 6 은 전량
s06 LINT-6 기존분). 프림: s03 2053→2056 · s05 1907→1907 · s06 1538→1522 ·
s10 3058→3059 · s15 743→723. 금지사항 준수 diff 검사: 사인·문안·사람·차량 **신설 0**.
**§4 렌더 라운드**: `260817_w4_regfix`(5씬) — **대기 중**. 사용자가 GPU 를 실사용
중이라(`env_seg` PID 4156230, 4916 MiB, `/tmp/negobs_gpu.lock` 보유) 라운드를
**씬 단위 락**으로 재구성해 큐에 넣었다(사용자 작업은 `flock -w 55` 라 라운드 전체를
한 번에 잠그면 exit 201 로 죽는다 — 씬 사이 틈을 내주는 구조). 육안·regr·gallery 는
렌더 완료 후 본 행에 추기한다.
**§4 렌더 검증 실측** (2026-08-16, round `260817_w4_regfix` 5씬 **71컷**,
건너뛴 씬 0 · 전 컷 stamp): 라운드는 **씬 단위 GPU 락**으로 돌렸다 — 사용자 `env_seg`
작업이 세 차례(00:18·00:55·02:26) 락을 점유했고 씬 사이 틈으로 정상 인터리브됐다
(s06 은 58분 대기 후 획득, `-w 3600` 상한 내). 컷수 s03 16 · s05 13 · s06 15 ·
s10 14 · s15 13.
**육안 판정 — 5씬 전건 성립**:
① **s05 성립(수치 확정)**. 감사가 지목한 4개 영역의 **순흑(lum<0.02) 비율이 전부
0.00 %** 로 소멸: `blade_L` 0.36→**0.00** · `joint_L` 2.51→**0.00** · `joint_R`
4.91→**0.00** · `blade_R` 0.47→**0.00**, 최저 휘도 0.000 → **0.179/0.472/0.061/0.541**.
= 바닥없는 흑슬롯·칼날 밑 흑슬릿이 실제로 닫혔다(잔존 어두운 쐐기는 음영 받은
실면이지 공극이 아님 — 최저휘도가 증명). 좌측 Z 균열 육안 소멸.
② **s10 성립(수치 확정)**. 수평선 **아래** 하늘계열 픽셀(공극 시그니처 RGB
0.552/0.592/0.633)이 5컷 전부 **0.00 %**: h1.8_d2 6.52→**0.00** · h0.9_d2 1.86→**0.00** ·
h1.8_d5 1.32→**0.00** · h1.8_d10 0.53→**0.00** · h0.9_d5 0.06→**0.00**.
서브에이전트의 사전 광선추적 예측(4319/1787/1333/387/48 px → 0)과 **렌더 실측이 일치**.
북사면 하부 암부 쐐기(뱅크 밑면)도 동일 박스로 동반 소멸 — 육안상 연속 잔디 지형.
③ **s06 성립**. 드럼 이음 캡 타원 4개가 사라지고 **단일 매끈 테이퍼 기둥**으로 판독.
데크·난간·유리 육안 불변.
④ **s03 성립**. 원수면 톱니 실루엣 소멸 — 원단이 **직선 수평 에지**로 판독.
⑤ **s15 성립**. 3프레임 shingle 캐스케이드 소멸, 창이 벽면에 분리 배치로 판독.
**regr 판정** (`regression_check.py --before-round 260816_w4_final33_on
--after-round 260817_w4_regfix`): **FAIL 5 · WARN 8**, 전량 **선언 귀속**.
- s10 **FAIL 5**(broken_rail·preset_h0.9_d5·h1.8_d2·h1.8_d5·reversal) + WARN 5 —
  전부 `FRAME` 유형 = **공동 폐합으로 화면의 큰 면적이 하늘→잔디로 정상 전환**된 것.
  선언(②)이 예고한 바로 그 변화이며 신규 결함 아님.
- s15 WARN 1(bend_landing, FRAME) = 개구부 재배치 선언 귀속.
- s03 WARN 1 · s06 WARN 1 — 둘 다 `UNCHANGED` 유형이고 s06 `spiral_up` 은
  **HEAD 기준선에서도 동일하게 관측**(GT-128 w8deck 대조에서 확인) = 기존분.
...
원형 복구는 보행면 z 이동을 수반하므로 별행 필요) ⓓ s15 좁은 4동 창 2→1 **사용자
판정 대기** ⓔ round_stamp 의 `env_captured:false` — 렌더 env 미기록(X2 구멍, 별건).
**Status: OPEN (착지 — 갤러리 검수 대기)**
