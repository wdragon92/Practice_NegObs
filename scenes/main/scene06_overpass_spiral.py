...
PARAMS = dict(
    ...
    #   (deck.grid_x 3.5).
    # [GT-101] cy -13 -> -16: the tower slides 3.0 m SOUTH with the corridor
    #   extension — the deck-post V (3.5,-9) east leg head sat at r 4.49 against
    #   the widened rim 4.5 / guard 4.44 / bottom treads (overlap the user saw);
    #   new centre distance 7.62 -> clearance 3.1 m, road edge (y -8) to rim 3.5 m.
    spiral=dict(cx=6.5, cy=-16.0, r_in=1.5, r_out=4.5, a0=180.0, sweep=300.0,
    ...
    # [GT-95 · 08-10 user] the circular landing is GONE — dict deleted, not disabled.
    #   History: GT-6 clipped the full north-half annulus to two lobes; GT-94 deleted the
    #   route-less east lobe; the user then judged even the west quarter connector awkward
    #   ("1/4 섹터도 제거하고, 계단을 통로 끝부분에 바로 연결하면 안 되나?"), so the stair
    #   top now meets the deck directly (spiral.a0 = the deck west-edge ray) and the deck's
    #   SW corner is bevelled along that ray (`build_deck`). The 4.166 m² residual-overlap
    #   debt GT-6 declared dies with the annulus.
    ...
    deck=dict(x0=2.0, x1=5.0, y0=-16.0, y1=13.0, z_top=5.0, thick=0.35,  # [GT-101] y0 -13->-16
...
# [GT-95] `_landing_arcs` / `_in_landing` deleted with the landing (GT-6 lobes → GT-94
...
def _deck_rail_y():
    """Y of the deck-rail / spiral-outer-guard shared newel, and the rail's bay boundaries.

    [GT-76] Derivation unchanged: the guard line r 3.24 leaves the deck edge x = x0 / x1
    at y = cy + sqrt(outer_r^2 - half^2) = **-10.128**, and the run is cut there so the
    arriving run shares ONE newel instead of crossing mid-bay. [GT-95] What arrives is now
    the spiral outer guard itself (west edge; the landing is gone): the WEST run has no
    apron bay — it starts at this newel, where the stair takes over. The EAST run keeps
    its apron return bay (y0 → newel) over the surviving straight deck end."""
...
def _solid_at(x, y, z):
    ...
    for tag, ya, yb in (("Verge_S", wk["ys0"] - vg["w"], wk["ys0"]),
        ...
        if vg["x0"] <= x <= vg["x1"] and ya <= y <= yb \
            ...
            return tag
    # ── spiral·column ── ([GT-95] landing branch deleted with the landing)
...
def _obstacle_boxes():
    ...
    yj_w = _deck_rail_y()[0]
    for i, xe in enumerate((dk["x0"], dk["x1"])):
        # [GT-95] the WEST run starts at the hand-over newel (no apron over the bevel)
        boxes.append((f"DeckRail_{i}", xe - 0.09, xe + 0.09,
                      yj_w if i == 0 else dk["y0"], dk["y1"],
    ...
    #   exclusion. The `DeckRail_i` box above still spans the whole slab edge: GT-76 cut the
    #   RUN into an apron return bay plus 13 regular bays, it did not shorten it.
    return boxes
...
def _smoke_report():
    ...
    rows = [
        ...
        # the number of steps between the top of step 25 and the top of step 0 is (n−1).
        #   [GT-95] the last riser is carried by "단 0 → 데크(직결)" (0.192) — the deck IS
        #   the final landing now; ground → deck rises 5.005 m over 26 risers + the direct
        #   step, no intermediate landing.
        ("나선 상행(디딤 25단차)", _spiral_top_z(sp["n"]-1), _spiral_top_z(0)),
        ("단 0 → 데크(직결)", _spiral_top_z(0), dk["z_top"]),
    ...
    d1, d2 = abs(cap_sp - cap_dk), abs(cap_no - cap_dk)
...
def build_views():
    ...
    out["spiral_up"] = dict(eye=[4.70, -14.20, 1.45], tgt=[8.30, -14.20, 2.75])
    ...
    out["deck_entry"] = dict(eye=[3.50, -12.50, 6.60], tgt=[3.95, -16.30, 4.90])
    ...
    out["broken_rail"] = dict(eye=[1.58, -19.44, 5.60],
                              tgt=[4.17, -18.33, 4.16])
...
[체크리스트]
 ...
 7. 데크 난간      — 유리 베이 + 연속 캡 · 종단 3곳 뉴얼(연단/랜딩/북측) 마감
...
def main():
    ...
    def build_deck(M):
        ...
        BOX(f"{ROOT}/Deck",
            ...
            M["deck"], col=True)
        # [GT-101 2판 · 12차] end skirts — in side view the thin slab lip (soffit
        #   4.65) jogged proud of the deeper stair fascia band ("바닥면 라인도
        #   연결부가 삐죽"). Two skirt bands drop the slab's exposed south/west
        #   edges to the fascia's bottom line (tread0 top − fascia thick ≈ 4.19),
        #   so the silhouette's lower line runs continuously into the ring.
        z_fb = (_spiral_z_at(PARAMS["spiral"]["a0"])
                + PARAMS["spiral"]["riser"] / 2.0 - 0.002
                - PARAMS["fascia"]["thick"])
        BOX(f"{ROOT}/DeckSkirtS",
            ((dk["x0"]+dk["x1"])/2.0, dk["y0"] + 0.125,
             (z_fb + dk["z_top"] - dk["thick"])/2.0),
            (dk["x1"]-dk["x0"], 0.25,
             dk["z_top"] - dk["thick"] - z_fb), M["concrete"])
        yj_rim = PARAMS["spiral"]["cy"] + math.sqrt(
            max(0.0, PARAMS["spiral"]["r_out"]**2
                - (PARAMS["spiral"]["cx"] - dk["x0"])**2))
        BOX(f"{ROOT}/DeckSkirtW",
            (dk["x0"] + 0.125, (dk["y0"] + yj_rim)/2.0,
             (z_fb + dk["z_top"] - dk["thick"])/2.0),
            (0.25, yj_rim - dk["y0"],
             dk["z_top"] - dk["thick"] - z_fb), M["concrete"])
    ...
    def build_cues(M):
        ...
        if cfg["cue_railing"]:
            ...
            rb, fa = PARAMS["rail_bay"], PARAMS["fascia"]
            bays = _guard_bays()
            gs, gl = _guard_section(True), _guard_section(False)
            ...
            # [GT-101 · 08-11 user] the GT-100 level-bay + knuckle came back jagged
            #   ("결국엔 삐죽삐죽"). The OUTER run now takes a C1-EASED walking line:
            #   slope 0 at the deck joint, blending into the helix over two bays — a
            #   hand on the cap never leaves it ("난간을 손으로 잡고 간다고 생각").
            # [GT-101 2판 · 12차] the smooth quadratic opened pane-top slivers: the
            #   cap band follows z_of as a CURVE while each pane top is a straight
            #   chord — up to ~50 mm of daylight mid-bay. The ease is therefore
            #   PIECEWISE-LINEAR at the bay boundaries (quadratic sampled at posts):
            ...
            def _ease_out_z(a):
                ...
                for (b0_, z0_), (b1_, z1_) in zip(ease_pts, ease_pts[1:]):
                    if a <= b1_ + 1e-9:
                        f = (a - b0_) / (b1_ - b0_)
