...
PARAMS = dict(
    ...
    # [규격] 핸드레일 0.85~0.90 m · 난간 1.10 m (편의증진법 시행규칙 별표1 · 건축법 시행령 §40)
    #   x1 = −0.90 : 점형블록 동단(−0.875)에서 0.30 m 수평 연장한 뒤 종단하는 규정 배치.
    tactile=dict(x0=-1.275, x1=-0.875, y_half=9.0, proud=0.004),
...
GATED_ZONES = [
    ...
    ("Tactile",      "cue_tactile",        -1.275, -0.875, -9.10,  9.10, 0.004),
...
def main():
    ...
    def build_railing(M):
        ...
        print(f"[cue] railing ON — 양측 난간 {n}본×2 (피치 {r['post_pitch']} m) · "
              ...
              f"· 동단 x={r['x1']:+.2f} (점형블록에서 0.30 m 수평 연장) "
    ...
    def build_tactile(M):
        ...
        print(f"[cue] tactile ON — 점형블록 세로 {t['x1'] - t['x0']:.2f} m · "
              f"계단 상단에서 {P['landing']['x1'] - t['x1']:.2f} m 이격 · "
              f"proud {t['proud']} m [규격 교통약자법 시행규칙 별표2]")
