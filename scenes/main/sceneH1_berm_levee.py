...
표준법 (법1–법8) 준수 메모
  ...
  법4 기능 필수성      — 난간(2.57 m 낙차 = 건축법 시행령 §40 의무 구간) · 볼라드(제방 진입
...
CUE_CLASS = {
    ...
    "cue_tactile":         "decorative",   # 갓돌 후방 0.30 m 점형블록 (proud 4 mm)
...
PARAMS = dict(
    ...
    # [규격] 난간 유효높이 1.10 m — 건축법 시행령 §40(낙차 1.20 m 초과 구간 의무).
    ...
    tactile=dict(x0=-1.32, x1=-0.92, y_half=8.6, proud=0.004),
    # [규격] 점형블록 세로 0.40 m · 갓돌 앞 0.30 m 이격 — 교통약자법 시행규칙 별표2.
    # [개정 R3] **마루 위 계단 상단 점형블록** — 교통약자법 시행규칙 별표2 는 계단
    #   **상단**에도 점형블록을 의무화한다. 마루 코(x = −4.05)에서 **0.10 m 안쪽**에
    #   앉혀 종단 모서리의 주인이 `BermCrest` 로 남게 한다(VG-06):
    #   마루 코를 스치는 시선은 x = −4.15 에서 1.140 + 0.135·0.10 = 1.1535 를 지나므로
    #   점형블록 상면 1.144 를 **항상 넘는다** ⇒ 점형블록이 코를 가릴 수 없다(구성상).
    #   포장 밴드(|y| ≤ 2.30) 위에만 깔린다 — 잔디 위 점형블록은 시공되지 않는다.
    crest_tactile=dict(x0=-4.55, x1=-4.15, proud=0.004),
...
GATED_ZONES = [
    ...
    # 점형블록: 갓돌 앞 0.30 m 규정 위치가 스트립 동단(x −1.15)과 겹친다.
    #   상면 융기는 proud 0.004 m 이므로 최악 판정은 `datum_tol`(≤0.02) 이고 격리 대상이 아니다.
    #   이것이 VG-datum 이 3층으로 설계된 바로 그 사례다 — 사전 선언하고 렌더 후 실측한다.
    ("Tactile",      "cue_tactile",        -1.32,  -0.92,  -8.70,  8.70, 0.004),
    ("TactileCrest", "cue_tactile",        -4.55,  -4.15,  -2.30,  2.30, 0.004),
...
def main():
    ...
    def build_tactile(M):
        ...
        # [개정 R3] 마루 위 **계단 상단** 점형블록 — 포장 밴드 위에만. 마루 코에서
        #   0.10 m 안쪽이라 종단 모서리의 주인은 `BermCrest` 로 남는다(PARAMS 주석의 검산).
        ...
        print(f"[cue] tactile ON — 갓돌 앞 점형블록 세로 {t['x1'] - t['x0']:.2f} m "
              f"(이격 {HZ['coping']['x0'] - t['x1']:.2f} m) + **마루 계단 상단** "
              f"점형블록 세로 {ct['x1'] - ct['x0']:.2f} m (마루 코에서 "
              f"{T['crest']['x1'] - ct['x1']:.2f} m 안쪽) · proud {t['proud']} m "
              "[규격 교통약자법 시행규칙 별표2]")
