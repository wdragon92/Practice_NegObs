"""Qualitative panels: RGB frame + GT grid + predicted-probability grid (3 bands x 5 sectors).

Picks --n detected-H, --n missed-H and --n off-frame false alarms from a per_frame.csv.
Grid is drawn far-band-on-top (row "3 far" .. "1 near"), columns A..E left-to-right.
"""
...
from polar_dataset import N_SECTORS  # noqa: E402


def _grid(ax, vals, title, tau, binary):
    m = np.asarray(vals, float).reshape(3, N_SECTORS)[::-1]  # far band on top
    ax.imshow(m, vmin=0, vmax=1, cmap="magma", aspect="auto")
    ax.set_title(title, fontsize=9)
    ax.set_xticks(range(N_SECTORS), list("ABCDE"), fontsize=8)
    ax.set_yticks(range(3), ["3 far", "2 mid", "1 near"], fontsize=8)
    for r in range(3):
...
def panel(rec_rgb, probs, gt, meta, out_png, tau):
    ...
    _grid(axes[1], gt, "GT", tau, True)
    _grid(axes[2], probs, f"pred prob (tau={tau})", tau, False)
...
def main(argv=None):
    ...
    for tag, idxs in (("hit" + a.tier, hit), ("miss" + a.tier, miss), ("fa_off", off)):
        ...
        for r, i in enumerate(idxs[: a.n], 1):
            ...
            panel(rgb.get(fid, ""), P[i], d["gt"][i], meta, out, a.tau)
            made += 1
    print(f"[viz] wrote {made} panels -> {a.out}")
    return 0
