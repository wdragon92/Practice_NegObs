...
**`[W2-D 개정]` 위 통과 조건표를 `260730_w2d_judge` 대조군으로 재기준화한다** (W2-C GO 항목 2 ·
결함 D13 — 낡은 절대치로 스프레드를 판정하면 실패를 제조한다). 근거·측정 전문은
`Docs/reports/w2d_round_v1.md` §5. 측정 규약은 §2.2 그대로(h0.3 3컷 중앙값 ·
`imgstats.analyze(lower=False)` · `w80` 은 `near_ground_stats`), scene19 `r2_on` 이
공표치 −2.66 / 0.028 / 10.0 을 그대로 재현함을 확인한 뒤 적용했다 `[실측]`.

| 씬 | v1.1 조건 | 본 라운드 실측 | **재기준화 조건** |
|---|---|---:|---|
| scene19 | `w80 < 40` | **0.00** | **달성 — 조건 폐지** |
| scene19 | `sat > 0.10` | **0.1074** | **달성 — 조건 폐지** |
| scene19 | `slope > −2.45` | −2.8298 | **`slope > −2.60`**(톤 작업이 반대로 밀었다) |
| scene19 | `flat_gnd` 10.0 에서 악화 없음 | **94.81** | **`flat_gnd < 20` — 이 씬의 차단 조건으로 승격** |
| scene07 | `slope ≥ −2.25` | −2.2913 | **유지** (0.036 부족, 사정권) |
| scene07 | `sat < 0.24` | 0.2503 | **`sat < 0.26`** (구 기준선은 대조군에서 0.0036 — 변별력 0) |
| scene07 | `flat_gnd` 1.7 악화 없음 | **0.73** | **`flat_gnd < 1.0`** |
| scene14 | `flat_gnd < 20` | **18.63** | **달성 → `flat_gnd < 15`** |
| scene14 | slope 목표대 이동 | −2.0004 | **달성**(`imgstats` −2.2…−2.0 대역 안) |
| scene21(대안) | `flat_gnd < 26` | **9.91** | **달성 → `flat_gnd < 12`** |
| sceneC2 | slope **−1.75** 이하로 더 가팔라지지 않을 것 | −2.0246 | **−2.10 이하로 더 가팔라지지 않을 것** |

**D13 종결**: sceneC2 slope 는 병합 후 대조군과 본 라운드에서 −2.02 로 같다(W2-D 콘텐츠
라운드 전체의 Δ **−0.0058**, T1 자체 기여는 −0.0003). −1.75 선은 낙엽 전역화·잔디 교체
이전 값이라 **재질층이 아니라 콘텐츠를 재고 있었다.** 새 선은 대조군 + 작업여유 0.08.

**★ 분기 종결**: scene14 는 `flat_gnd` 31.3 → **18.63** 으로 v1.1 조건을 **통과**한다.
D11(V자 톱니)은 본 라운드에서도 재확인됐으나(`w2d_round_v1.md` §4.3) 판정 지표 자체는
합격이므로 **분기 A/B 는 scene14 유지로 닫는다** — scene21 로의 교체는 불필요하다.
