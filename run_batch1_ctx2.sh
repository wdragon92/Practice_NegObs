#!/bin/bash
# ============================================================================
# 배치1 맥락 라운드(ctx2) 검증 체인 — 씬 위치: scenes/batch1/
#   ① SMOKE 12씬 직렬 (실패 시 렌더 중단)
#   ② RT 전 12씬 → look_check/<id>/ctx2
#   ③ PT: D4(실내 판정) 전 뷰 + C1·C4(overcast, RT 평면광 보완) 서브셋
#      → look_check/<id>/ctx2_pt
#   로그: look_check/logs/batch1_ctx2.log
# ============================================================================
...
LOG=../../look_check/logs/batch1_ctx2.log
: > $LOG
...
for s in $SCENES; do
  id=${s%%_*}
  NEGOBS_CAPTURE=1 NEGOBS_CAPTURE_MODE=rt NEGOBS_CAPTURE_DIR=../../look_check/${id}/ctx2 \
    timeout 900 python $s.py >> $LOG 2>&1
  echo "$s RT_EXIT=$? FILES=$(ls ../../look_check/${id}/ctx2 2>/dev/null | wc -l)" | tee -a $LOG
done

echo "== [3/3] PT 판정 ==" | tee -a $LOG
NEGOBS_CAPTURE=1 NEGOBS_CAPTURE_MODE=pt NEGOBS_WARMUP=900 \
  NEGOBS_CAPTURE_DIR=../../look_check/sceneD4/ctx2_pt \
  timeout 2400 python sceneD4_subway_platform.py >> $LOG 2>&1
echo "sceneD4 PT_EXIT=$? FILES=$(ls ../../look_check/sceneD4/ctx2_pt 2>/dev/null | wc -l)" | tee -a $LOG
NEGOBS_CAPTURE=1 NEGOBS_CAPTURE_MODE=pt NEGOBS_WARMUP=600 \
  NEGOBS_VIEWS="approach,grazing_top,lower_lookback" \
  NEGOBS_CAPTURE_DIR=../../look_check/sceneC1/ctx2_pt \
  timeout 2400 python sceneC1_snow_stairs.py >> $LOG 2>&1
echo "sceneC1 PT_EXIT=$? FILES=$(ls ../../look_check/sceneC1/ctx2_pt 2>/dev/null | wc -l)" | tee -a $LOG
NEGOBS_CAPTURE=1 NEGOBS_CAPTURE_MODE=pt NEGOBS_WARMUP=600 \
  NEGOBS_VIEWS="approach,grazing_mirror,film_closeup" \
  NEGOBS_CAPTURE_DIR=../../look_check/sceneC4/ctx2_pt \
  timeout 2400 python sceneC4_wet_stairs.py >> $LOG 2>&1
echo "sceneC4 PT_EXIT=$? FILES=$(ls ../../look_check/sceneC4/ctx2_pt 2>/dev/null | wc -l)" | tee -a $LOG
echo "batch1 ctx2 완료" | tee -a $LOG
