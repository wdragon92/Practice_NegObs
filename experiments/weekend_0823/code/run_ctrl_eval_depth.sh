...
#   eval_new_depth_s{42,43,44}  eval_polar on the NEW pair            (GPU, one lock hold each)
#   eval_old_depth_s{42,43,44}  the same metrics recomputed on the FROZEN v2 eval rows,
...
#   CTRL_TABLE_DEPTH.md         the side-by-side table + ctrl_numbers.json
#
...
ON_ROUND="$REPO/dataset/260819_main_on"
CTRL_ROUND="$REPO/dataset/260820_ctrloff"
...
for S in $SEEDS; do
  CK="$V2/depth_s$S/best.pt"
  FROZEN_PF="$V2/depth_s$S/eval_test/per_frame.csv"
  NEW="$HERE/eval_new_depth_s$S"; OLD="$HERE/eval_old_depth_s$S"
  ...
  if [ ! -f "$NEW/metrics.json" ]; then
    gpu_step "eval_new s$S" \
      "$PY $CODE/eval_polar.py --manifest '$HERE/manifest_ctrl.json' --split '$HERE/split_ctrl.json' \
         --subset test --input depth --ckpt '$CK' --out '$NEW' --tau-op $TAU --tau-star $TAU \
  ...
  step "twin new s$S" \
    "$CPU $CODE/twin_analysis.py --per-frame-on '$NEW/per_frame_on.csv' \
       ...
       --out '$HERE/twin_new_depth_s$S' --tol $TOL --grid $CODE/labeling/$GRID"
...
PY"
  ...
  step "twin old s$S" \
    "$CPU $CODE/twin_analysis.py --per-frame-on '$OLD/per_frame_on.csv' \
       ...
       --out '$HERE/twin_old_depth_s$S' --tol $TOL --grid $CODE/labeling/$GRID"
done
...
if [ -z "$FAILED" ]; then
  ...
  say "  table: $HERE/CTRL_TABLE_DEPTH.md"
  exit 0
fi
