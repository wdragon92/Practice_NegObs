...
CUE_CLASS = {
    ...
    # [D82 ⓐ 근거 조항] 「교통약자의 이동편의증진법 시행규칙」 별표2 —
...
PARAMS = dict(
    terrain=dict(
        ...
        # 점형블록 띠 — 세로 0.60 m [규격 교통약자법 시행규칙 별표2 · ADA §705 610 mm]
    ...
    #        (「교통약자의 이동편의증진법 시행규칙」 별표2). 실물보다 큰 소품이 되면
    ...
    tactile=dict(
        # (a) **점형블록(경고) 1밴드** — 승차 위치.
        #   [규격 「교통약자의 이동편의증진법 시행규칙」 별표2 — 버스 승강장의 **승차
        ...
        #   [규격 동 별표2 — 선형블록은 **유도** 목적이며 돌출선(4줄/유닛)은 **보행
    ...
    #   [규격 「도로안전시설 설치 및 관리지침 — 보행자용 방호울타리편」 h 1.00~1.20 m,
    #    지주 간격 1.5~2.0 m. 목적 = ①추락 방지 ②횡단 금지 ③보행 안전 **병렬 규정**]
    ...
    def build_tactile(M):
        ...
        **[개정 R3 · D82 ⓐ]** 역할 구분을 기하로 강제한다:
          · **점형(경고)** = `sc.build_tactile`(도트형) · 승차 위치 1.80 × 0.60 m,
            연석 배면에서 **0.30 m 이격**. [별표2 — 버스 승강장 승차 위치 / ADA §705.2]
          · **선형(유도)** = `_linear_tactile()` · 보도 → 승차 위치 0.30 × 1.42 m,
            **돌출선이 보행 방향(−y)과 나란하다**. [별표2 — 선형블록 = 유도]
        ...
        print(f"[cue] tactile ON — **점형(경고) 1밴드** {wx1 - wx0:.2f} × "
              ...
              "[규격 교통약자법 시행규칙 별표2 · ADA §705.2/§810 · D82 ⓐⓒ]")

    def _linear_tactile(M, path, x0, y0, x1, y1, z, t):
        """**선형블록(유도)** — 바탕판 + 평행 돌출선. 점형(도트)과 **기하가 다르다**.

        [규격 별표2] 선형블록의 돌출선은 유닛(0.30 m)당 **4줄**이고 **보행 방향과
...
def _geometry_selfcheck():
    ...
    chk("점형 밴드 깊이 ≥ 0.60 m [별표2 승강장 · ADA §705.2 610 mm]",
        tw[3] - tw[1] >= 0.60 - 1e-9, f"{tw[3] - tw[1]:.2f} m")
    chk("점형 밴드가 연석 배면에서 **0.30 m 이격** [별표2 위험 지점 0.3 m 앞]",
    ...
    #   않는다 ⇒ 별표2 제7호의 "말뚝 앞 0.3 m 점형블록"은 **적용되지 않는다**.
    #   (횡단 배치는 sceneN11 이고 그 씬은 점형블록을 둔다.) D82 부가 지침
    #   *"규정 미적용 시점에 규정 시설물을 억지로 두지 말 것"* 의 이행이다.
    chk("볼라드 열이 보행 동선과 **나란하다** ⇒ 별표2 제7호 점형블록 미적용",
