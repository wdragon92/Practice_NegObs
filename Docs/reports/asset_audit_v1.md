...
| oakfall2 | 0.0179 m² | ~~21.1 %~~ **30.1 %** | ~~0.0038~~ **0.0054 m²** | **185.2개** |

> ⚠ **2026-07-29 재정정(W1)**: oakfall2 행은 usd-core 독립 재래스터화로 **0.0054 m²** 확정
> (`Docs/surveys/props_audit_w1/B_groundcover_debris.md` §6 — 래스터라이저 해석 검증, redteam_w1_assets 승소).
> 낱장 maplefall1(→0.0081)·oakfall1(→0.0048)·클러스터 2행(→0.0584/0.0239)도 계통 편차 확인 —
> 코드 반영은 렌더 영향이 있는 fallcluster 행 때문에 W2 낙엽 전역화 게이트에서 일괄 처리.

**폴리곤 예산** — ⚠ **2026-07-29 정정** (레드팀 `redteam_verification_v1.md` R7).
초판의 아래 문장은 **면적 전제가 틀렸고 결론("감당 가능")도 성립하지 않는다.**

> ~~sceneC2 낙엽 밴드가 대략 6 m² 이므로 완전피복은 ~880k 삼각형. 현행 낙엽
> 타원체 900개(≈115k 삼각형)의 8배지만, 관목 하나(Forsythia 404k)보다 적으므로
> **감당 가능**하다.~~

| 항목 | 초판 | **정정** | 근거 |
|---|---|---|---|
| 산포 밴드 면적 | 6 m² | **46.02 m²** (7.7배) | `[실측]` |
| 완전피복 시 | ~880k 삼각형 | **6.72M 삼각형** | `[계산]` |
| 현행 코드가 실제로 만드는 양 | (미산출) | **5,225,315 삼각형** | `[실측]` |
| Forsythia(404k) 대비 | "더 적다" | **12.9배 많다** | `[계산]` |

- **단위 비용은 그대로다**: fallcluster1 완전피복 15.9개/m² × 9,175 =
  **146k 삼각형/m²** `[실측 — §4-b]`.
- **면적 전제가 오기였다.** 실제 3D 산포 영역은 `sceneC2_leaf_stairs.py` 의
  `leaf_scatter`(`x0` −4.20 · `x_pad_near` 1.50 · `y_near` 2.20)와
  `RUN = TREAD × NSTEP = 4.76` 에서 **x −4.20…6.26 × y ±2.20 =
  10.46 × 4.40 = 46.02 m²** 다 `[실측 — 씬 파라미터 직접 계산]`.
  (텍스처까지 포함한 전체 낙엽 영역은 86.11 m² 이고 3D 밴드는 그 53.4 % 다.)
- 그 면적의 완전피복은 146k × 46.02 = **6.72M 삼각형** `[계산]`.
- **현행 호출이 실제로 만드는 양**(`cover=0.55` · `pool=fallcluster` 2종 ·
  `max_count=900`): n = 46.02·(−ln 0.45)/0.0435 = **845개**
  (추첨 결과 fallcluster1 437 + fallcluster2 408) →
  437×9,175 + 408×2,980 = **5,225,315 삼각형**
  `[실측 — scatter_debris 의 RNG 소비 순서를 재현해 산출. 레드팀 R5 수치와 일치]`.
- 따라서 초판의 비교 대상이 틀렸다: 실제는 초판이 쓴 880k 의 **5.9배**,
  Forsythia 한 주(404k)의 **12.9배**다 `[계산]`.
- 인스턴싱: 레드팀 R6(부모 프림에 `SetInstanceable` → 컴포지션 아크가 없어
  무동작)은 **`dc88f21` 에서 `/Asset` 대상으로 이미 수정됐다** `[코드]`.
  프로토타입 공유가 실제로 성립하는지는 Isaac 런타임 1줄 확인
  (`prim.IsInstance()` · `len(stage.GetPrototypes())`) 전까지 미검증이고,
  성립하더라도 줄어드는 것은 **고유 데이터(메모리·BVH 빌드)** 이지
  씬에 존재하는 인스턴스 845개와 그 유효 삼각형 5.23M 자체가 아니다 `[추정]`.
- 남는 선택지(수치만 제시, 코드 변경은 렌더 게이트 뒤): (a) `cover` 하향 —
  0.30 이면 n = 377개 · 약 2.3M `[계산]`, (b) 낱장(`oakfall`·`maplefall`,
  496~631 삼각형)을 섞어 개당 비용을 낮춘다, (c) 인스턴싱을 실제로 켜고
  재산정한다. 아래 3층 전략(§5-a)은 (b) 를 이미 담고 있으나, **그 스니펫의
  예산 문구 역시 이 정정된 면적 기준으로 다시 읽어야 한다.**
