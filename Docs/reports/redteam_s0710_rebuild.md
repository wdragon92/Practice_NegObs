...
| 6 | GT-14 R-2 | `check_data_run.py 260731_s07_recache` re-run; `dataset/260731_s07_recache` on disk (8 PNG + manifest) | [1]–[5] all PASS, 3.16 s/cut; the only 2 FAILs are the zero-sample cross-condition checks (`0 pairs ≥ 0.3 EV`, `0 scene-pairs`) — exactly as recorded verbatim in the ledger |
