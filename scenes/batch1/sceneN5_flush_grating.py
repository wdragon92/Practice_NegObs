...
Hazard (counter-example): **there is no drop anywhere.** A 0.4 m wide long drainage grating
           ...
           surface. Between the bars it "looks open", but in reality it is a **shallow dark zone**
           carried by a dark floor plate at z=−0.05, with no real opening (GT drop is 0, so an actual hole is forbidden).
...
PARAMS = dict(
    ...
    material=dict(
        scale=dict(plaza_light=1.1, concrete_wall=2.0, grass=1.4,
                   brick_red=2.0),
        # Walkway: plaza_light source mean sRGB 0.714 + a weak neutral tint -> ~0.64
        ...
        # Trench floor plate: a shallow dark zone (not an opening but a "floor that looks dark")
    ...
    #     Slot penetration check: bar top face (z=+0.001) -> floor plate (z=-0.050), depth 0.051 m.
    #       Horizontal travel = 0.051·cot(49.79 deg) = 0.0431 m, of which the component along the drain axis (62 deg)
    #       = 0.0431·|cos(25 deg-62 deg)| = 0.0344 m > slot gap 0.030 m
    #       => **direct light never reaches the floor plate** -> the slots read reliably as a dark zone
    #          (reproducing the black slits of reference n5). At the same time the walkway surface is bright under frontal light.
    SUN_AZ_OFFSET=171.5,
...
def main():
    ...
    def build_pave_and_grating(M):
        ...
        gx0, gx1 = px - hl, px + hl
        gxc = (gx0 + gx1) / 2.0
        # Trench floor plate (a shallow dark zone - not a real opening)
